"""
Card Position Calibration Tool

This script helps you find the correct screen coordinates for card positions
in your poker client. It will:
1. Take a screenshot of your poker table
2. Let you define card positions by entering coordinates
3. Show you a preview of each card region
4. Allow you to adjust until perfect
5. Output the final coordinates for config.yaml
"""
...
def get_coordinates(card_name, default_coords=None):
    """Get coordinates for a card position from user input."""
    print(f"\n--- {card_name.upper()} ---")
    if default_coords:
...
def preview_crop(image_path, card_name, coords, crop_dir="calibration_crops"):
    """Show a preview of the cropped region."""
    ...
    # Also save a larger preview for easier viewing
    preview_size = (crop.width * 4, crop.height * 4)  # 4x scale
    preview = crop.resize(preview_size, Image.NEAREST)  # Use NEAREST to keep pixels sharp
...
def create_annotated_screenshot(image_path, all_coords, output_path="calibration_crops/annotated_screenshot.png"):
    """Create an annotated version of the screenshot showing all card positions."""
    img = Image.open(image_path)
    draw = ImageDraw.Draw(img)

    # Draw rectangles and labels for each card
    colors = {
...
def calibrate_card_position(image_path, card_name, default_coords=None):
    """Calibrate a single card position with preview and adjustment loop."""
...
def format_yaml_output(all_coords):
    """Format coordinates as YAML for config file."""
    print("\n" + "="*60)
...
def main():
    ...
    # Load default coordinates from config if available
    default_coords = {
    ...
    # Create annotated screenshot showing all positions
