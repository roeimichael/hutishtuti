...
class VisualCalibrator:
    ...
    def draw_rectangle(self, coords, color='green'):
        ...
        self.rect_patch = patches.Rectangle((left, top), width, height, linewidth=2, edgecolor=color, facecolor='none')
        self.ax.add_patch(self.rect_patch)
        self.ax.text(left, top - 10, self.current_card.upper(), color=color, fontsize=12, fontweight='bold',
    ...
    def save_and_exit(self):
        ...
        for card_name, coords in self.coords.items():
            ...
            self.ax.text(left, top - 10, card_name.upper(), color=color, fontsize=10, fontweight='bold',
                        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
